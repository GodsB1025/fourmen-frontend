...
import { fetchCalendar, mapToEventInput, addCalendarEvent, updateCalendarEvent, deleteCalendarEvent } from "../../apis/Calendar";
...
const isMeeting = (ev: any) => (ev?.extendedProps?.event_type ?? ev?.event_type) === "meeting";
...
export default function ProfileCalendar({ onMonthChange }: Props) {
    ...
    const fetchEventsForDate = async (dateStr: string) => {
        ...
        const mapped = list.map(mapToEventInput).filter((ev: any) => !ev.__invalid && typeof ev.start === "string");

        const isYmd = (s: string) => /^\d{4}-\d{2}-\d{2}$/.test(s);
        const isAllDayByShape = (ev: any) => ev.allDay === true || (typeof ev.start === "string" && isYmd(ev.start));
    ...
    return (
        <div className="profile-cal">
            <FullCalendar
                ...
                eventClassNames={(arg) => (isMeeting(arg.event) ? ["evt-meeting"] : ["evt-personal"])}
                eventDidMount={(info) => {
    ...
    return (
        <div className="cal-create">
            <div className="cal-create-row">
                <label>제목</label>
                <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="예: 기업 미팅" />
            </div>
...
function EventList({
    ...
    const detectAllDay = (startIso: string, endIso?: string) => {
        ...
        return s.getHours() === 0 && s.getMinutes() === 0 && e.getHours() === 0 && e.getMinutes() === 0 && e.getTime() > s.getTime();
    };
                    ...
                    <li key={ev.id} className="cal-list-item">
                        {isEdit ? (
                            <div className="cal-create" style={{ width: "100%" }}>
                                <div className="cal-create-row">
                                    <label>제목</label>
                                    <input type="text" className="cal-title-input" value={title} onChange={(e) => setTitle(e.target.value)} />
                                </div>
                                ...
                                <div className="cal-actions" style={{ justifyContent: "flex-end", marginTop: "1rem" }}>
                                    <button className="cal-btn" onClick={cancelEdit}>
                                        취소
                                    </button>
                                    <button className="cal-btn cal-primary" onClick={() => saveEdit(ev)}>
                                        저장
                                    </button>
                                </div>
                            </div>
