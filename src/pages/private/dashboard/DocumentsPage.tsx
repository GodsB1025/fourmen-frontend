...
// --- 회의록 공유 모달 ---
// UPDATE: minute prop 제거 (내부에서 사용하지 않으므로 불필요했고 타입 충돌 유발)
...
const isMinuteDetail = (m: MinuteDetail | ManualMinuteResponse | null): m is MinuteDetail =>
  !!m && "meetingId" in m && "meetingTitle" in m && "type" in m;
...
const MinuteDetailModal = ({
  ...
  minute: MinuteDetail | ManualMinuteResponse; // 유니온
  onClose: () => void;
  ...
  const title = isMinuteDetail(minute) ? minute.meetingTitle : (minute as any).title ?? "수동 회의록";
  const author = (minute as any).authorName ?? "알 수 없음";
  const createdAt = (minute as any).createdAt ? format(new Date((minute as any).createdAt), "yyyy.MM.dd HH:mm") : "-";
  const typeBadge = isMinuteDetail(minute) ? minute.type.toLowerCase() : "self";
  const typeLabel = isMinuteDetail(minute) ? minuteTypeLabel[minute.type] : "수동 회의록";
  const content = (minute as any).content ?? "";
    ...
    <div className="document-modal-backdrop" onClick={onClose}>
      <div className="document-modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="document-modal-subheader">
          <span className={`badge type-${typeBadge}`}>{typeLabel}</span>
          <span className="meta">작성자: {author}</span>
          <span className="meta">작성일: {createdAt}</span>
        </div>
        <main className="document-modal-body markdown-body">
          <ReactMarkdown remarkPlugins={[remarkGfm]}>{content || "*내용이 없습니다.*"}</ReactMarkdown>
        </main>
        {/* 공유 버튼은 MinuteDetail일 때만 노출 */}
        {showShareButton && isMinuteDetail(minute) && (
...
export default function DocumentsPage() {
  ...
  const [viewingMinute, setViewingMinute] = useState<MinuteDetail | ManualMinuteResponse | null>(null);
  ...
  const handleViewMinute = async (meetingId: number, minuteId: number) => {
    ...
    try {
      const minuteDetails = await getMinuteDetails(String(meetingId), minuteId); // 기대: Promise<MinuteDetail | ManualMinuteResponse>
  ...
  const handleShareMinute = async (userIds: number[]) => {
    ...
    if (!isMinuteDetail(viewingMinute)) {
      alert("이 회의록은 회의 기반 문서가 아니라 공유 기능을 지원하지 않습니다.");
      return;
    }
    try {
  ...
  const filteredDocs = useMemo(() => {
    ...
    const filteredMeetingsWithDocs = meetingsWithDocs
      .map((dailyDocs) => {
  ...
  return (
    <div className="docs-page-layout">
      ...
      {/* UPDATE: ShareMinuteModal 호출 시 minute prop 제거 */}
      {viewingMinute && isShareModalOpen && (
        <ShareMinuteModal onClose={() => setIsShareModalOpen(false)} onShare={handleShareMinute} />
      )}
    </div>
  );
}
